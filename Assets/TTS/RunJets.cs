...
public class RunJets : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        ReadDictionary();
        TextToSpeech(false);
        Debug.Log("Dummy Execution Done");
    }
    ...
    public void TextToSpeech(bool isPlay = true)
    {
        string ptext;
        if (hasPhenomeDictionary)
        {
            string koreanRoamnized = KoreanRomanizer.ConvertToRomanization(inputText);
            Debug.Log(koreanRoamnized);
            ptext = TextToPhonemes(koreanRoamnized);
            Debug.Log(ptext);
        }
        else
        {
    ...
    public async void DoInference(string ptext, bool isPlay)
    {      
        ...
        var samples = cpuOutput.DownloadToArray();
    ...
    public void SetInputText(TMP_InputField inputField)
    {
